...
@section('title')
	<title>Legal Job Search, Solicitor Vacancies, Legal recruitment agency</title>
...
@section('content')
    <section class="lqd-section banner flex items-center bg-cover bg-center border-10 border-white qc-sublanding" id="landing" style="background: linear-gradient(93deg, #01091278, #0b223e73), url('{{ url('frontend/assets/images/bg/top-banner-desktop-latest-jobs-v1.webp') }}') no-repeat center/cover;">
        <div class="container">
    ...
    <section class="lqd-section case-studies jshndle-section-fixed qc-x-section res-reset-height mainjobsearch flex items-center justify-center py-50 px-10vw bg-white lg:px-0 md:items-start" id="findjobsection" data-section-luminosity="light">
        <div class="container">
            <div class="row">
                    ...
                    <div class="qc-jobs-parent-container">
                        ...
                        <div class="qc-filter-container">
                            @foreach ($job_filters as $filter)
                                <div class="qc-filter">
                                    <select name="" id="" class="qc-form-select jshndle-job-filter">
                                        <option value="all" selected>All {{ $filter->title }}</option>
                ...
                @endphp
                @if ($premiumJobs->count() > 0)
                    <div class="col-lg-12 col-md-12 col-12 px-10 mb-10">
                        <div class="qc-featured-jobs-container">
                            <div class="swiper qc-featured-job-slider">
                                <div class="swiper-wrapper">
                                    @foreach ($premiumJobs as $job)
                                        @php
                                            $locationHtml = '';
                                            if ($job->location) {
                                                $locArr = array_filter(array_map('trim', explode(',', $job->location)));
                                                $shownLocs = implode(', ', array_slice($locArr, 0, 2));
                                                $moreCount = count($locArr) - 2;
                                                $locText = $moreCount > 0 ? $shownLocs . ', +' . $moreCount . ' more' : $shownLocs;
                                                $locationHtml = '<p class="qc-job-card-location badge mb-0" title="' . e($job->location) . '"><i class="fa-solid fa-location-dot"></i> ' . e($locText) . '</p>';
                                            }
                                        @endphp

                                        <div class="swiper-slide">
                                            <div class="qc-featured-job jshndle-job-card job-premium" data-job-id="{{ $job->id }}" data-qccode="{{ $job->qc_code_series }}">
                                                <div class="qc-premium-badge">
                                                    <i class="fa-solid fa-star"></i>
                                                </div>
                                                <div class="qc-job-card-header">
                                                    {!! $locationHtml !!}
                                                    <h3 class="qc-job-card-title text-20 mb-0 qc-fw-600">{{ $job->title }}</h3>
                                                </div>
                                            </div>
                                        </div>
                                    @endforeach
...
@section('script')
    ...
    <script>
        $(document).ready(function () {
            ...
            // qch: fetching filters from the url and search
            function fillAllFiltersFromURLAndSearch() {
                const params = new URLSearchParams(window.location.search);

                const jobTitle = params.get('jobtitle');
                const jobLocation = params.get('joblocation');
                const jobCategory = params.get('jobcategory');
                const jobPQE = params.get('jobpqe');
                const jobDescriptions = params.get('jobdescriptions');

                if (jobTitle) {
                    $('.jshndle-job-search-input').val(jobTitle);
                }

                const filterValues = [jobCategory, jobLocation, jobPQE, jobDescriptions];

                $('.jshndle-job-filter').each(function(index) {
                ...
                if (jobTitle || jobLocation || jobCategory || jobPQE || jobDescriptions) {
                    $('.qc-search-button').trigger('click');
                }
            }

            // qch: updating the url with current filters
            function updateURLWithCurrentFilters() {
                const params = new URLSearchParams();

                const jobTitle = $('.jshndle-job-search-input').val();
                if (jobTitle) {
                    params.set('jobtitle', jobTitle);
                }

                const filterKeys = ['jobcategory', 'joblocation', 'jobpqe', 'jobdescriptions'];

                $('.jshndle-job-filter').each(function(index) {
                    const value = $(this).val();
                    if (value && value.toLowerCase() !== 'all') {
                        params.set(filterKeys[index], value);
                    }
                });
            ...
            fillAllFiltersFromURLAndSearch();
                    ...
                    success: function(response) {
                        ...
                        let html = `
                            ...
                            <div class="qc-jobs-desc">
                                ...
                                <h3 class="ld-fh-element relative mb-0/5em text-20 leading-1/1em qc-text-primary qc-text-capitalize">
                                    ${response.title}
                                </h3>
                                <div class="job-overview-container job-common-container">
                                   ...
                                   ${response.position ? `
                                    ...
                                    ` : ''}
                                    ${response.qc_code_series ? `
                                        <div class="job-overview">
                                            <div class="label">
                                                <p class="my-0 qc-fw-600">QC Code:</p>
                                            </div>
                                            <div class="text">
                                                <p class="my-0">${response.qc_code_series}</p>
                                            </div>
                                        </div>
                                    ` : ''}
                                    ${response.pqe ? `
                                    ...
                                    ${response.type ? `
                                        <div class="job-overview">
                                            <div class="label">
                                                <p class="my-0 qc-fw-600">Job Type:</p>
                                            </div>
                                            <div class="text">
                                                <p class="my-0">${response.type}</p>
                                            </div>
                                        </div>
                                    ` : ''}
                                    ${response.pay_grade ? `
                                        ...
                                        <div role="form">
                                            ...
                                            <form class="lqd-cf-form jshndle-job-form">
                                                ...
                                                <div class="row -mr-15 -ml-15 flex-wrap">
                                                    ...
                                                    <div class="col col-12 col-md-6 px-15">
                                                        <div class="relative w-full qc-floating-label">
                                                            <input type="text" name="phone_number" id="phone_number" placeholder="" class="border-black-20 text-black text-13 qc-bg-transparent">
                                                            <label for="first_name" class="">Phone Number</label>
                                                        </div>
                                                    </div>
                                                    ...
                                                    <div class="col col-12 px-15 mb-10">
                                                        <label class="flex items-start qc-terms-check space-x-2">
                                                            ...
                                                            <span>By using this form you agree with the storage and handling of your data by this website. <span class="qc-text-danger">*</span></span>
                                                        </label>
                                                    </div>
            ...
            function renderJobs(jobs, replace = false) {
                ...
                jobs.forEach(job => {
                    let fields = [
                        ...
                        { value: job.qc_code_series, icon: 'fa-solid fa-barcode' },
                    ];
                        ...
                        <div class="qc-job-card jshndle-job-card job-common" data-job-id="${job.id}">
                            ...
                            <div class="qc-job-card-header">
                                <div class="d-flex">
                                    ...
                                    ${job.is_premium == 1 ? `<div class="qc-job-card-location badge qc-premium-badge"><i class="fa-solid fa-star"></i></div>` : ''}
                                </div>
                                ${job.title ? `<h3 class="qc-job-card-title qc-text-capitalize">${job.title}</h3>` : ''}
                            </div>
            ...
            $(document).on('click', '.jshndle-job-form-submit', function (event) {
                ...
                let $this = $(this);
                $this.html('<i class="fa-solid fa-loader fa-spin"></i>');
